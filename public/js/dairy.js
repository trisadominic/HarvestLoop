document.addEventListener('DOMContentLoaded', function() {
    ...
    // Example: Fetch dairy list with authentication
    const token = localStorage.getItem('token');
    fetch('/api/products/dairy', {
      headers: {
    ...
    .then(data => {
      ...
      const dairyGrid = document.getElementById('dairy-grid');
      const dairyCount = document.getElementById('dairy-count');
      dairyGrid.innerHTML = '';
      
      if (!Array.isArray(data)) {
        console.error('Expected array but got:', typeof data, data);
        dairyCount.textContent = 'Error loading dairy products';
        return;
      }
      
      dairyCount.textContent = `Showing ${data.length} dairy products`;

      data.forEach(dairy => {
        const card = document.createElement('div');
        card.className = 'dairy-card bg-white rounded-lg shadow-md p-6 flex gap-4';
        
        // Use actual farmer's image or a proper fallback
        const imageUrl = dairy.imageUrl && dairy.imageUrl !== '/assets/default-dairy.jpg' 
          ? dairy.imageUrl 
        ...
        card.innerHTML = `
          <div class="flex-1">
            <h2 class="text-xl font-bold mb-2">${dairy.name}</h2>
            <p class="text-gray-600 mb-2">Price: <span class="font-semibold">Rs. ${dairy.price}</span> / ${dairy.stockUnit || 'liter'}</p>
            <p class="text-gray-600 mb-2">Quantity: ${dairy.quantity}</p>
            <p class="text-gray-600 mb-2">${dairy.description}</p>
            <p class="text-sm text-gray-400 mb-4">Farmer: ${dairy.farmerId?.username || 'Unknown'}</p>
            
            <div class="flex gap-2">
              <button class="buy-btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-dairy-id="${dairy._id || dairy.id || 'unknown'}" data-dairy-name="${dairy.name}" data-dairy-price="${dairy.price}" data-dairy-unit="${dairy.stockUnit || 'liter'}" data-dairy-quantity="${dairy.quantity}">
                <i class="fas fa-shopping-cart mr-1"></i>Buy
              </button>
              <button class="deal-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-dairy-id="${dairy._id || dairy.id || 'unknown'}" data-dairy-name="${dairy.name}" data-dairy-price="${dairy.price}" data-dairy-unit="${dairy.stockUnit || 'liter'}" data-dairy-quantity="${dairy.quantity}">
                <i class="fas fa-handshake mr-1"></i>Deal
              </button>
              <button class="farmer-btn bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-farmer-id="${dairy.farmerId?._id || dairy.farmerId}" data-farmer-name="${dairy.farmerId?.username || 'Unknown'}" data-dairy-name="${dairy.name}">
                <i class="fas fa-user mr-1"></i>Farmer
              </button>
            </div>
          </div>
          <div class="w-32 h-32 flex-shrink-0">
            <img src="${imageUrl}" alt="${dairy.name}" class="w-full h-full object-cover rounded" 
      ...
      document.querySelectorAll('.buy-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          const dairyId = this.dataset.dairyId;
          const dairyName = this.dataset.dairyName;
          const dairyPrice = this.dataset.dairyPrice;
          const dairyUnit = this.dataset.dairyUnit;
          const dairyQuantity = this.dataset.dairyQuantity; // Available stock
          
          console.log('Dairy data:', { dairyId, dairyName, dairyPrice, dairyUnit, dairyQuantity });
          
          showPurchasePopup(dairyName, dairyPrice, dairyUnit, dairyId, dairyQuantity);
        });
      });
      ...
      document.querySelectorAll('.deal-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Deal data:', { dairyId, dairyName, dairyPrice, dairyUnit, dairyQuantity });
          
          showDealPopup(dairyName, dairyPrice, dairyUnit, dairyId, dairyQuantity);
        });
      });
      ...
      document.querySelectorAll('.farmer-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Farmer data:', { farmerId, farmerName, dairyName });
          
          showFarmerModal(farmerId, farmerName, dairyName);
        });
      });
    })
    .catch(error => {
      ...
      const dairyCount = document.getElementById('dairy-count');
      dairyCount.textContent = 'Error loading dairy products';
      dairyGrid.innerHTML = '<div class="text-center text-red-500">Failed to load dairy products. Please try again later.</div>';
    });
});
...
function showPurchasePopup(dairyName, dairyPrice, dairyUnit, dairyId, availableQuantity) {
  console.log('showPurchasePopup called with:', { dairyName, dairyPrice, dairyUnit, dairyId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Purchase ${dairyName}</h2>
        <button onclick="closePurchasePopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Confirm the quantity you wish to purchase at the list price of ₹${dairyPrice} / ${dairyUnit}.</p>
      <p class="text-sm text-blue-600 mb-4"><i class="fas fa-info-circle mr-1"></i>Available Stock: ${availableQuantity} ${dairyUnit}</p>
      
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">Quantity (${dairyUnit})</label>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center">
          ...
          <span id="totalCost" class="text-green-600 font-bold text-xl">₹${dairyPrice}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="confirmButton" onclick="confirmPurchase('${dairyId}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
  ...
  setTimeout(() => {
    ...
    if (quantityInput) {
      quantityInput.addEventListener('input', function() {
        validateAndUpdateQuantity(dairyPrice, dairyUnit, availableQuantity);
      });
    }
  }, 100);
}
...
function confirmPurchase(dairyId) {
  const quantity = document.getElementById('purchaseQuantity').value;
  const token = localStorage.getItem('token');
  
  closePurchasePopup();
  
  fetch(`/api/subscriptions/unlock-farmer/${dairyId}`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
...
function showDealPopup(dairyName, dairyPrice, dairyUnit, dairyId, availableQuantity) {
  console.log('showDealPopup called with:', { dairyName, dairyPrice, dairyUnit, dairyId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Negotiate for ${dairyName}</h2>
        <button onclick="closeDealPopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Propose a new price or send a message to the farmer. The list price is Rs.${dairyPrice} / ${dairyUnit}.</p>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Your Offer (Rs. / ${dairyUnit})</label>
          <input type="number" id="dealPrice" value="${Math.round(dairyPrice * 0.9)}" min="1" step="0.01" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
        </div>
      </div>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center mb-2">
          ...
          <span id="listPriceTotal">Rs.${dairyPrice}</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-green-600 font-semibold">Your Offer Total</span>
          <span id="offerTotal" class="text-green-600 font-semibold">Rs.${Math.round(dairyPrice * 0.9)}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="sendOfferButton" onclick="sendDealOffer('${dairyId}')" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
          Send Offer
        </button>
      </div>
    </div>
  ...
  function updateTotals() {
    ...
    const listPrice = parseFloat(dairyPrice);
    ...
    if (quantity > availableQuantity) {
      quantityError.classList.remove('hidden');
      quantityErrorMessage.textContent = `Maximum available: ${availableQuantity} ${dairyUnit}`;
...
function sendDealOffer(dairyId) {
...
function showFarmerModal(farmerId, farmerName, dairyName) {
