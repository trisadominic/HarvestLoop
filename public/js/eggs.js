document.addEventListener('DOMContentLoaded', function() {
    ...
    // Example: Fetch eggs list with authentication
    const token = localStorage.getItem('token');
    fetch('/api/products/eggs', {
      headers: {
    ...
    .then(data => {
      ...
      if (!Array.isArray(data)) {
        ...
        eggCount.textContent = 'Error loading eggs';
        return;
      }
      
      eggCount.textContent = `Showing ${data.length} egg products`;

      data.forEach(egg => {
        const card = document.createElement('div');
        card.className = 'egg-card bg-white rounded-lg shadow-md p-6 flex gap-4';
        
        // Use actual farmer's image or a proper fallback
        const imageUrl = egg.imageUrl && egg.imageUrl !== '/assets/default-egg.jpg' 
          ? egg.imageUrl 
        ...
        card.innerHTML = `
          <div class="flex-1">
            <h2 class="text-xl font-bold mb-2">${egg.name}</h2>
            <p class="text-gray-600 mb-2">Price: <span class="font-semibold">Rs. ${egg.price}</span> / ${egg.stockUnit || 'dozen'}</p>
            <p class="text-gray-600 mb-2">Quantity: ${egg.quantity}</p>
            <p class="text-gray-600 mb-2">${egg.description}</p>
            <p class="text-sm text-gray-400 mb-4">Farmer: ${egg.farmerId?.username || 'Unknown'}</p>
            
            <div class="flex gap-2">
              <button class="buy-btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-egg-id="${egg._id || egg.id || 'unknown'}" data-egg-name="${egg.name}" data-egg-price="${egg.price}" data-egg-unit="${egg.stockUnit || 'dozen'}" data-egg-quantity="${egg.quantity}">
                <i class="fas fa-shopping-cart mr-1"></i>Buy
              </button>
              <button class="deal-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-egg-id="${egg._id || egg.id || 'unknown'}" data-egg-name="${egg.name}" data-egg-price="${egg.price}" data-egg-unit="${egg.stockUnit || 'dozen'}" data-egg-quantity="${egg.quantity}">
                <i class="fas fa-handshake mr-1"></i>Deal
              </button>
              <button class="farmer-btn bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-farmer-id="${egg.farmerId?._id || egg.farmerId}" data-farmer-name="${egg.farmerId?.username || 'Unknown'}" data-egg-name="${egg.name}">
                <i class="fas fa-user mr-1"></i>Farmer
              </button>
            </div>
          </div>
          <div class="w-32 h-32 flex-shrink-0">
            <img src="${imageUrl}" alt="${egg.name}" class="w-full h-full object-cover rounded" 
      ...
      document.querySelectorAll('.buy-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          const eggName = this.dataset.eggName;
          const eggPrice = this.dataset.eggPrice;
          const eggUnit = this.dataset.eggUnit;
          const eggQuantity = this.dataset.eggQuantity; // Available stock
          
          console.log('Egg data:', { eggId, eggName, eggPrice, eggUnit, eggQuantity });
          
          showPurchasePopup(eggName, eggPrice, eggUnit, eggId, eggQuantity);
        });
      });
      ...
      document.querySelectorAll('.deal-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Deal data:', { eggId, eggName, eggPrice, eggUnit, eggQuantity });
          
          showDealPopup(eggName, eggPrice, eggUnit, eggId, eggQuantity);
        });
      });
      ...
      document.querySelectorAll('.farmer-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Farmer data:', { farmerId, farmerName, eggName });
          
          showFarmerModal(farmerId, farmerName, eggName);
        });
      });
    })
    .catch(error => {
      ...
      const eggCount = document.getElementById('egg-count');
      eggCount.textContent = 'Error loading eggs';
      eggGrid.innerHTML = '<div class="text-center text-red-500">Failed to load eggs. Please try again later.</div>';
    });
});
...
function showPurchasePopup(eggName, eggPrice, eggUnit, eggId, availableQuantity) {
  console.log('showPurchasePopup called with:', { eggName, eggPrice, eggUnit, eggId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Purchase ${eggName}</h2>
        <button onclick="closePurchasePopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Confirm the quantity you wish to purchase at the list price of ₹${eggPrice} / ${eggUnit}.</p>
      <p class="text-sm text-blue-600 mb-4"><i class="fas fa-info-circle mr-1"></i>Available Stock: ${availableQuantity} ${eggUnit}</p>
      
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">Quantity (${eggUnit})</label>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center">
          ...
          <span id="totalCost" class="text-green-600 font-bold text-xl">₹${eggPrice}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="confirmButton" onclick="confirmPurchase('${eggId}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
  ...
  setTimeout(() => {
    ...
    if (quantityInput) {
      quantityInput.addEventListener('input', function() {
        validateAndUpdateQuantity(eggPrice, eggUnit, availableQuantity);
      });
    }
  }, 100);
}
...
function confirmPurchase(eggId) {
  const quantity = document.getElementById('purchaseQuantity').value;
  const token = localStorage.getItem('token');
  
  closePurchasePopup();
  
  fetch(`/api/subscriptions/unlock-farmer/${eggId}`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
...
function showDealPopup(eggName, eggPrice, eggUnit, eggId, availableQuantity) {
  console.log('showDealPopup called with:', { eggName, eggPrice, eggUnit, eggId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Negotiate for ${eggName}</h2>
        <button onclick="closeDealPopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Propose a new price or send a message to the farmer. The list price is Rs.${eggPrice} / ${eggUnit}.</p>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Your Offer (Rs. / ${eggUnit})</label>
          <input type="number" id="dealPrice" value="${Math.round(eggPrice * 0.9)}" min="1" step="0.01" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
        </div>
      </div>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center mb-2">
          ...
          <span id="listPriceTotal">Rs.${eggPrice}</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-green-600 font-semibold">Your Offer Total</span>
          <span id="offerTotal" class="text-green-600 font-semibold">Rs.${Math.round(eggPrice * 0.9)}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="sendOfferButton" onclick="sendDealOffer('${eggId}')" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
          Send Offer
        </button>
      </div>
    </div>
  ...
  function updateTotals() {
    ...
    const listPrice = parseFloat(eggPrice);
    ...
    if (quantity > availableQuantity) {
      quantityError.classList.remove('hidden');
      quantityErrorMessage.textContent = `Maximum available: ${availableQuantity} ${eggUnit}`;
...
function sendDealOffer(eggId) {
  console.log('🎯 sendDealOffer called with eggId:', eggId);
...
function showFarmerModal(farmerId, farmerName, eggName) {
