document.addEventListener('DOMContentLoaded', function() {
    ...
    // Example: Fetch grain list with authentication
    const token = localStorage.getItem('token');
    fetch('/api/products/grains', {
      headers: {
    ...
    .then(data => {
      ...
      if (!Array.isArray(data)) {
        ...
        grainCount.textContent = 'Error loading grains';
        return;
      }
      
      grainCount.textContent = `Showing ${data.length} grains`;

      data.forEach(grain => {
        const card = document.createElement('div');
        card.className = 'grain-card bg-white rounded-lg shadow-md p-6 flex gap-4';
        
        // Use actual farmer's image or a proper fallback
        const imageUrl = grain.imageUrl && grain.imageUrl !== '/assets/default-grain.jpg' 
          ? grain.imageUrl 
        ...
        card.innerHTML = `
          <div class="flex-1">
            <h2 class="text-xl font-bold mb-2">${grain.name}</h2>
            <p class="text-gray-600 mb-2">Price: <span class="font-semibold">Rs. ${grain.price}</span> / ${grain.stockUnit || 'kg'}</p>
            <p class="text-gray-600 mb-2">Quantity: ${grain.quantity}</p>
            <p class="text-gray-600 mb-2">${grain.description}</p>
            <p class="text-sm text-gray-400 mb-4">Farmer: ${grain.farmerId?.username || 'Unknown'}</p>
            
            <div class="flex gap-2">
              <button class="buy-btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-grain-id="${grain._id || grain.id || 'unknown'}" data-grain-name="${grain.name}" data-grain-price="${grain.price}" data-grain-unit="${grain.stockUnit || 'kg'}" data-grain-quantity="${grain.quantity}">
                <i class="fas fa-shopping-cart mr-1"></i>Buy
              </button>
              <button class="deal-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-grain-id="${grain._id || grain.id || 'unknown'}" data-grain-name="${grain.name}" data-grain-price="${grain.price}" data-grain-unit="${grain.stockUnit || 'kg'}" data-grain-quantity="${grain.quantity}">
                <i class="fas fa-handshake mr-1"></i>Deal
              </button>
              <button class="farmer-btn bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-farmer-id="${grain.farmerId?._id || grain.farmerId}" data-farmer-name="${grain.farmerId?.username || 'Unknown'}" data-grain-name="${grain.name}">
                <i class="fas fa-user mr-1"></i>Farmer
              </button>
            </div>
          </div>
          <div class="w-32 h-32 flex-shrink-0">
            <img src="${imageUrl}" alt="${grain.name}" class="w-full h-full object-cover rounded" 
      ...
      document.querySelectorAll('.buy-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          const grainId = this.dataset.grainId;
          const grainName = this.dataset.grainName;
          const grainPrice = this.dataset.grainPrice;
          const grainUnit = this.dataset.grainUnit;
          const grainQuantity = this.dataset.grainQuantity; // Available stock
          
          console.log('Grain data:', { grainId, grainName, grainPrice, grainUnit, grainQuantity });
          
          showPurchasePopup(grainName, grainPrice, grainUnit, grainId, grainQuantity);
        });
      });
      ...
      document.querySelectorAll('.deal-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Deal data:', { grainId, grainName, grainPrice, grainUnit, grainQuantity });
          
          showDealPopup(grainName, grainPrice, grainUnit, grainId, grainQuantity);
        });
      });
      ...
      document.querySelectorAll('.farmer-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Farmer data:', { farmerId, farmerName, grainName });
          
          showFarmerModal(farmerId, farmerName, grainName);
        });
      });
    })
    .catch(error => {
      ...
      const grainCount = document.getElementById('grain-count');
      grainCount.textContent = 'Error loading grains';
      grainGrid.innerHTML = '<div class="text-center text-red-500">Failed to load grains. Please try again later.</div>';
    });
...
function showPurchasePopup(grainName, grainPrice, grainUnit, grainId, availableQuantity) {
  console.log('showPurchasePopup called with:', { grainName, grainPrice, grainUnit, grainId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Purchase ${grainName}</h2>
        <button onclick="closePurchasePopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Confirm the quantity you wish to purchase at the list price of ₹${grainPrice} / ${grainUnit}.</p>
      <p class="text-sm text-blue-600 mb-4"><i class="fas fa-info-circle mr-1"></i>Available Stock: ${availableQuantity} ${grainUnit}</p>
      
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">Quantity (${grainUnit})</label>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center">
          ...
          <span id="totalCost" class="text-green-600 font-bold text-xl">₹${grainPrice}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="confirmButton" onclick="confirmPurchase('${grainId}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
  ...
  setTimeout(() => {
    ...
    if (quantityInput) {
      quantityInput.addEventListener('input', function() {
        validateAndUpdateQuantity(grainPrice, grainUnit, availableQuantity);
      });
    }
  }, 100);
}
...
function confirmPurchase(grainId) {
  const quantity = document.getElementById('purchaseQuantity').value;
  const token = localStorage.getItem('token');
  ...
  fetch(`/api/subscriptions/unlock-farmer/${grainId}`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
...
function showDealPopup(grainName, grainPrice, grainUnit, grainId, availableQuantity) {
  console.log('showDealPopup called with:', { grainName, grainPrice, grainUnit, grainId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Negotiate for ${grainName}</h2>
        <button onclick="closeDealPopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Propose a new price or send a message to the farmer. The list price is Rs.${grainPrice} / ${grainUnit}.</p>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Your Offer (Rs. / ${grainUnit})</label>
          <input type="number" id="dealPrice" value="${Math.round(grainPrice * 0.9)}" min="1" step="0.01" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
        </div>
      </div>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center mb-2">
          ...
          <span id="listPriceTotal">Rs.${grainPrice}</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-green-600 font-semibold">Your Offer Total</span>
          <span id="offerTotal" class="text-green-600 font-semibold">Rs.${Math.round(grainPrice * 0.9)}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="sendOfferButton" onclick="sendDealOffer('${grainId}')" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
          Send Offer
        </button>
      </div>
    </div>
  ...
  function updateTotals() {
    ...
    const listPrice = parseFloat(grainPrice);
    ...
    if (quantity > availableQuantity) {
      quantityError.classList.remove('hidden');
      quantityErrorMessage.textContent = `Maximum available: ${availableQuantity} ${grainUnit}`;
...
function sendDealOffer(grainId) {
  console.log('🎯 sendDealOffer called with grainId:', grainId);
...
function showFarmerModal(farmerId, farmerName, grainName) {
  console.log('showFarmerModal called with:', { farmerId, farmerName, grainName });
...
// Functions for static negotiate modal
...
function sendNegotiateOffer() {
  const modal = document.getElementById('negotiateModal');
  const productId = modal.dataset.productId;
  const quantity = document.getElementById('quantity').value;
  const offerPrice = document.getElementById('yourOffer').value;
  const message = document.getElementById('message').value;
  const token = localStorage.getItem('token');
  
  console.log('sendNegotiateOffer called with:', { productId, quantity, offerPrice, message });
