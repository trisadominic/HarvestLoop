document.addEventListener('DOMContentLoaded', function() {
    ...
    // Example: Fetch vegetable list with authentication
    const token = localStorage.getItem('token');
    fetch('/api/products/vegetables', {
      headers: {
    ...
    .then(data => {
      ...
      const vegetableCount = document.getElementById('vegetable-count');
      vegetableGrid.innerHTML = '';
      
      if (!Array.isArray(data)) {
        console.error('Expected array but got:', typeof data, data);
        vegetableCount.textContent = 'Error loading vegetables';
        return;
      }
      
      vegetableCount.textContent = `Showing ${data.length} vegetables`;

      data.forEach(vegetable => {
        const card = document.createElement('div');
        card.className = 'vegetable-card bg-white rounded-lg shadow-md p-6 flex gap-4';
        
        // Use actual farmer's image or a proper fallback
        const imageUrl = vegetable.imageUrl && vegetable.imageUrl !== '/assets/default-vegetable.jpg' 
          ? vegetable.imageUrl 
        ...
        card.innerHTML = `
          <div class="flex-1">
            ...
            <p class="text-gray-600 mb-2">Price: <span class="font-semibold">Rs. ${vegetable.price}</span> / ${vegetable.stockUnit || 'kg'}</p>
            <p class="text-gray-600 mb-2">Quantity: ${vegetable.quantity}</p>
            <p class="text-gray-600 mb-2">${vegetable.description}</p>
            <p class="text-sm text-gray-400 mb-4">Farmer: ${vegetable.farmerId?.username || 'Unknown'}</p>
            
            <div class="flex gap-2">
              <button class="buy-btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-vegetable-id="${vegetable._id || vegetable.id || 'unknown'}" data-vegetable-name="${vegetable.name}" data-vegetable-price="${vegetable.price}" data-vegetable-unit="${vegetable.stockUnit || 'kg'}" data-vegetable-quantity="${vegetable.quantity}">
                <i class="fas fa-shopping-cart mr-1"></i>Buy
              </button>
              <button class="deal-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-vegetable-id="${vegetable._id || vegetable.id || 'unknown'}" data-vegetable-name="${vegetable.name}" data-vegetable-price="${vegetable.price}" data-vegetable-unit="${vegetable.stockUnit || 'kg'}" data-vegetable-quantity="${vegetable.quantity}">
                <i class="fas fa-handshake mr-1"></i>Deal
              </button>
              <button class="farmer-btn bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm font-medium transition-colors" data-farmer-id="${vegetable.farmerId?._id || vegetable.farmerId}" data-farmer-name="${vegetable.farmerId?.username || 'Unknown'}" data-vegetable-name="${vegetable.name}">
                <i class="fas fa-user mr-1"></i>Farmer
              </button>
            </div>
          </div>
          <div class="w-32 h-32 flex-shrink-0">
            <img src="${imageUrl}" alt="${vegetable.name}" class="w-full h-full object-cover rounded" 
      ...
      document.querySelectorAll('.buy-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          const vegetableId = this.dataset.vegetableId;
          const vegetableName = this.dataset.vegetableName;
          const vegetablePrice = this.dataset.vegetablePrice;
          const vegetableUnit = this.dataset.vegetableUnit;
          const vegetableQuantity = this.dataset.vegetableQuantity; // Available stock
          
          console.log('Vegetable data:', { vegetableId, vegetableName, vegetablePrice, vegetableUnit, vegetableQuantity });
          
          showPurchasePopup(vegetableName, vegetablePrice, vegetableUnit, vegetableId, vegetableQuantity);
        });
      });
      ...
      document.querySelectorAll('.deal-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Deal data:', { vegetableId, vegetableName, vegetablePrice, vegetableUnit, vegetableQuantity });
          
          showDealPopup(vegetableName, vegetablePrice, vegetableUnit, vegetableId, vegetableQuantity);
        });
      });
      ...
      document.querySelectorAll('.farmer-btn').forEach(button => {
        button.addEventListener('click', function(e) {
          ...
          console.log('Farmer data:', { farmerId, farmerName, vegetableName });
          
          showFarmerModal(farmerId, farmerName, vegetableName);
        });
      });
    })
    .catch(error => {
      ...
      const vegetableCount = document.getElementById('vegetable-count');
      vegetableCount.textContent = 'Error loading vegetables';
      vegetableGrid.innerHTML = '<div class="text-center text-red-500">Failed to load vegetables. Please try again later.</div>';
    });
...
function showPurchasePopup(vegetableName, vegetablePrice, vegetableUnit, vegetableId, availableQuantity) {
  console.log('showPurchasePopup called with:', { vegetableName, vegetablePrice, vegetableUnit, vegetableId, availableQuantity });
  ...
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Purchase ${vegetableName}</h2>
        <button onclick="closePurchasePopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Confirm the quantity you wish to purchase at the list price of ₹${vegetablePrice} / ${vegetableUnit}.</p>
      <p class="text-sm text-blue-600 mb-4"><i class="fas fa-info-circle mr-1"></i>Available Stock: ${availableQuantity} ${vegetableUnit}</p>
      
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">Quantity (${vegetableUnit})</label>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center">
          ...
          <span id="totalCost" class="text-green-600 font-bold text-xl">₹${vegetablePrice}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="confirmButton" onclick="confirmPurchase('${vegetableId}')" class="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
  ...
  setTimeout(() => {
    ...
    if (quantityInput) {
      quantityInput.addEventListener('input', function() {
        validateAndUpdateQuantity(vegetablePrice, vegetableUnit, availableQuantity);
      });
    }
  }, 100);
}
...
function confirmPurchase(vegetableId) {
  const quantity = document.getElementById('purchaseQuantity').value;
  const token = localStorage.getItem('token');
  ...
  fetch(`/api/subscriptions/unlock-farmer/${vegetableId}`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
  ...
  .then(data => {
    if (data.farmer) {
      ...
      if (data.productUpdated) {
        // Refresh the product list to show updated quantities/availability
        setTimeout(() => {
          location.reload(); // Simple refresh - could be optimized to just update specific product
        }, 2000);
      }
    } else {
...
function showDealPopup(vegetableName, vegetablePrice, vegetableUnit, vegetableId, availableQuantity) {
  console.log('showDealPopup called with:', { vegetableName, vegetablePrice, vegetableUnit, vegetableId, availableQuantity });
  
  // Use the static modal from HTML
  const modal = document.getElementById('negotiateModal');
  const productNameSpan = document.getElementById('productName');
  const listPriceSpan = document.getElementById('listPriceValue');
  const quantityInput = document.getElementById('quantity');
  const offerInput = document.getElementById('yourOffer');
  const messageInput = document.getElementById('message');
  const listTotalSpan = document.getElementById('listTotal');
  const yourTotalSpan = document.getElementById('yourTotal');
  
  // Set the product details
  productNameSpan.textContent = vegetableName;
  listPriceSpan.textContent = `₹${vegetablePrice} / ${vegetableUnit}`;
  
  // Reset form
  quantityInput.value = '1';
  quantityInput.max = availableQuantity;
  offerInput.value = Math.round(vegetablePrice * 0.9);
  messageInput.value = '';
  
  // Store product info for sending deal
  modal.dataset.productId = vegetableId;
  modal.dataset.productName = vegetableName;
  modal.dataset.productPrice = vegetablePrice;
  modal.dataset.productUnit = vegetableUnit;
  ...
  quantityInput.addEventListener('input', updateOfferTotals);
  offerInput.addEventListener('input', updateOfferTotals);
  ...
  function updateOfferTotals() {
    ...
    const offerPrice = parseFloat(offerInput.value) || 0;
    const listTotal = (parseFloat(vegetablePrice) * quantity).toFixed(2);
    const offerTotal = (offerPrice * quantity).toFixed(2);
    
    listTotalSpan.textContent = `₹${listTotal}`;
    yourTotalSpan.textContent = `₹${offerTotal}`;
  }
}
  popup.innerHTML = `
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Negotiate for ${vegetableName}</h2>
        <button onclick="closeDealPopup()" class="text-gray-500 hover:text-gray-700">
          <i class="fas fa-times text-xl"></i>
        </button>
      </div>
      
      <p class="text-gray-600 mb-4">Propose a new price or send a message to the farmer. The list price is Rs.${vegetablePrice} / ${vegetableUnit}.</p>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Your Offer (Rs. / ${vegetableUnit})</label>
          <input type="number" id="dealPrice" value="${Math.round(vegetablePrice * 0.9)}" min="1" step="0.01" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
        </div>
      </div>
      ...
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <div class="flex justify-between items-center mb-2">
          ...
          <span id="listPriceTotal">Rs.${vegetablePrice}</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-green-600 font-semibold">Your Offer Total</span>
          <span id="offerTotal" class="text-green-600 font-semibold">Rs.${Math.round(vegetablePrice * 0.9)}</span>
        </div>
      ...
      <div class="flex gap-3">
        ...
        <button id="sendOfferButton" onclick="sendDealOffer('${vegetableId}')" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg font-medium transition-colors">
          Send Offer
        </button>
      </div>
    </div>
  ...
  function updateTotals() {
    ...
    const listPrice = parseFloat(vegetablePrice);
    ...
    if (quantity > availableQuantity) {
      quantityError.classList.remove('hidden');
      quantityErrorMessage.textContent = `Maximum available: ${availableQuantity} ${vegetableUnit}`;
...
function sendDealOffer(vegetableId) {
  console.log('🎯 sendDealOffer called with vegetableId:', vegetableId);
...
function showFarmerModal(farmerId, farmerName, vegetableName) {
  console.log('showFarmerModal called with:', { farmerId, farmerName, vegetableName });
